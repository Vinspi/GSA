...
//	    aliquotRepository.save(newAliquot);
//	    
//		for(Transaction transactionAliquotExpire : outdatedAliquot) {
...
//	  }
	...
	public boolean updateAliquotExpire(long id) {
		...
		if (aliquotExpire.isPresent()) {
			...
			if (dateExpire.isBefore(currentDate)) {
				...
				newAliquot.setAliquotQuantityHiddenStock(0);
				aliquotRepository.save(newAliquot);
				List<Transaction> transactionAliquotExpires = transactionRepository
						.findOutdatedTransactionByAliquot(id);
	...
	public boolean isExpire(long id) {
		Optional<Aliquot> aliquotExpire = aliquotRepository.findById(id);
		if (aliquotExpire.isPresent()) {
			Aliquot newAliquot = aliquotExpire.get();
			LocalDate dateExpire = newAliquot.getAliquotExpirationDate();
...
//findbyidandtransactionLike findbytranstypelike --> apres .out
	// utiliser JPA
	/*
	 * public Transaction getTransactionByAliquot(Aliquot aliquot) { return
	 * transactionRepository.findByAliquot(aliquot); }
	 * 
	 * public Iterator<Transaction>
	 * transformAliquotsToTransactions(Collection<Aliquot> aliquots) { return
	 * getTransactionStreamFromBdd(aliquots).iterator(); }
	 * 
	 * private Stream<Transaction> getTransactionStreamFromBdd(Collection<Aliquot>
	 * aliquots) { return aliquots.stream().map(this::getTransactionByAliquot); }
	 * 
	 * @Override public List<Transaction>
	 * getTransactionsByAliquots(Collection<Aliquot> aliquots) { return
	 * getTransactionStreamFromBdd(aliquots).collect(Collectors.toList()); }
	 */
